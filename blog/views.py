...
def result(request):
    ...
    engine=sqlalchemy.create_engine('mysql+pymysql://root:Jashboy@06@localhost/stockdb', pool_recycle=3600)
    conn=engine.connect()
    predictor=Predictor()
    dataset=predictor.readsql(stock.lower(),conn)
    livedata=predictor.get_live_data(stock)
    dataset=predictor.mergedata(dataset,livedata)
    predictor.writetosql(stock.lower(),dataset,conn)
